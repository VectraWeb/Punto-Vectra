...
// Vocabulario canónico de formas: inglés (es lo que entiende SalonFloor).
// El editor y datos viejos pueden traer español → se traduce al leer.
const SHAPE_EN = ['rectangular', 'round', 'square', 'square-sm'];
const SHAPE_ES_TO_EN = {
  redonda: 'round',
  rectangular: 'rectangular',
  cuadrada: 'square',
  'cuadrada chica': 'square-sm',
};

export function canonShape(s) {
  if (s == null) return null;
  const str = String(s).trim();
  if (SHAPE_EN.includes(str)) return str;
  const low = str.toLowerCase();
  if (SHAPE_EN.includes(low)) return low;
  return SHAPE_ES_TO_EN[low] || null;
}
...
export function normalizeResource(doc, opts = {}) {
  ...
  return {
    ...
    // shape: canónico en inglés (lo que entiende SalonFloor).
    // Traduce español legacy ("cuadrada" → "square") al leer.
    shape: canonShape(raw.shape ?? raw.forma) || RESOURCE_SHAPE_BY_TYPE[type] || 'rectangular',
...
export function resourceToMesa(resource) {
  return {
    ...
    shape: canonShape(resource.shape) || RESOURCE_SHAPE_BY_TYPE[resource.type] || 'rectangular',
