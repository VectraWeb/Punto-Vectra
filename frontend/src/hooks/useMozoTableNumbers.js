...
export function useMozoTableNumbers(tables, staff, sectors, positions = {}) {
  return useMemo(() => {
    ...
    // Números directos de la DB (resource.number) — siempre disponibles.
    for (const t of tds) {
      if (t.number) tableNumByTable[t.id] = String(t.number);
    }

    // Asignación mozo→mesa vía sectores (sigue funcionando para el plano).
