...
export default function ResModal({
  ...
  const [form, setForm] = useState(() => {
    ...
    // En edición, el backend guarda resourceId (no tableId): usarlo como mesa actual.
    if (editing && !base.tableId) {
      base.tableId = editing.resourceId || editing.mesaId || editing.mesa_id || '';
    }
  ...
  const svcForTime = serviceFromTime(form.time, service, services);
  const timeInOtherService = form.time && svcForTime !== service;

  // Al asignar un mozo, no auto-seleccionar mesa: mesa y mozo son independientes.
            ...
            <select value={form.tableId} onChange={e => set('tableId', e.target.value)} style={inp}>
              <option value="">— elegir —</option>
               {tables.filter(t => {
                  ...
                  const isCurrentRes = form.tableId && form.tableId === t.id;
