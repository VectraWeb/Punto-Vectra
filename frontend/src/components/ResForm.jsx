...
import { C, serviceFromTime, defaultServiceTime, todayISO, servicesOf } from '../utils';
...
export default function ResForm({ onStaffAccess, onBack, organization: organizationProp }) {
  ...
  const [form, setForm] = useState({
    ...
    phone: '',
    partySize: 2,
    time: defaultServiceTime(servicesOf(organization)),
    notes: '',
    metadata: {},
  });
  ...
  const service = serviceFromTime(form.time, undefined, SVC);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data: freshOrg } = await organizationApi.getById(organization.id);
      const freshClosed = Array.isArray(freshOrg.closedDates) ? freshOrg.closedDates : [];
      if (freshClosed.includes(date)) {
        setError('Este día estamos cerrados. Por favor elegí otra fecha.');
        setSubmitting(false);
        return;
      }
    } catch (e) {
      console.error('Error verificando disponibilidad:', e);
      setError('No se pudo verificar la disponibilidad. Intentá de nuevo.');
      setSubmitting(false);
      return;
    }
    ...
    try {
      await setDoc(resDocRef(id), {
        ...
        staffName: '',
        time: form.time,
        duration: SVC[service].defaultDuration,
        service,
        notes: form.notes,
        mesa_id: null,
        resourceId: null,
      ...
      setTimeout(() => {
        setSuccess(false);
        setForm({ customerName: '', phone: '', partySize: 2, time: defaultServiceTime(SVC), notes: '', metadata: {} });
      }, 3000);
    } catch (e) {
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        {service && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', padding: '10px 14px', background: '#fdf6e3', border: `1px solid ${C.soon}`, borderRadius: '12px', color: '#6b5a00', lineHeight: '1.4' }}>
            ...
            <span><strong>{serviceLabel}</strong> · de {SVC[service].start} a {SVC[service].end}</span>
          </div>
        )}
        ...
        {timeOutOfRange && (
          <div style={{ padding: '10px 14px', background: '#fdf6e3', border: `1px solid ${C.soon}`, borderRadius: '12px', fontSize: '13px', color: '#6b5a00', lineHeight: '1.4' }}>
            Ese horario está fuera de nuestra atención: {serviceLabelOf(organization, 'mediodia')} de {SVC.mediodia.start} a {SVC.mediodia.end} y {serviceLabelOf(organization, 'cena')} de {SVC.cena.start} a {SVC.cena.end}.
          </div>
        )}
