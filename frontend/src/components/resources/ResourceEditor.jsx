...
import { C, SHAPE_KEYS, SHAPE_LABELS, SHAPE_MAP } from '../../utils';
...
export default function ResourceEditor({ organization = null, onClose }) {
  ...
  // Cambios optimistas: se muestran al instante y se descartan cuando
  // el servidor los confirma (llega el valor por polling/refresco).
  const [overrides, setOverrides] = useState({});

  // Solo se aplica el override mientras el servidor todavía trae el valor
  // viejo; cuando el refresco confirma el cambio, se usa el valor real.
  const visible = useMemo(
    () => resources.map(r => {
      const ov = overrides[r.id];
      if (!ov) return r;
      const pending = Object.fromEntries(Object.entries(ov).filter(([k, v]) => r[k] !== v));
      return Object.keys(pending).length ? { ...r, ...pending } : r;
    }),
    [resources, overrides]
  );
  ...
  const handleUpdate = async (r, patch) => {
    setOverrides(prev => ({ ...prev, [r.id]: { ...(prev[r.id] || {}), ...patch } }));
    try {
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginTop: '6px' }}>
                ...
                {r.type === 'table' && (
                  <>
                    ...
                    >
                      {SHAPE_KEYS.map(k => <option key={k} value={SHAPE_MAP[k] || k}>{SHAPE_LABELS[k]}</option>)}
                    </select>
                  </>
                )}
              </div>
