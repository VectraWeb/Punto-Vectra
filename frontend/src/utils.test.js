...
import { t2m, m2t, genSlots, buildTables, todayISO, detectService, SERVICES, DEFAULT_CONFIG, timeBelongsToService, serviceFromTime, servicesOf } from './utils.js';
...
describe('servicesOf (horarios configurables)', () => {
  it('devuelve los defaults sin organización ni config', () => {
    expect(servicesOf(null)).toBe(SERVICES);
    expect(servicesOf({})).toBe(SERVICES);
    expect(servicesOf({ configuration: {} })).toBe(SERVICES);
  });

  it('mezcla horarios custom con defaults', () => {
    const org = { configuration: { services: { mediodia: { start: '12:00', end: '16:00' } } } };
    const svc = servicesOf(org);
    expect(svc.mediodia.start).toBe('12:00');
    expect(svc.mediodia.end).toBe('16:00');
    expect(svc.mediodia.name).toBe('Mediodía');
    expect(svc.cena.start).toBe(SERVICES.cena.start);
  });

  it('ignora valores inválidos', () => {
    const org = { configuration: { services: { cena: { start: 'nope', end: '' } } } };
    const svc = servicesOf(org);
    expect(svc.cena.start).toBe(SERVICES.cena.start);
    expect(svc.cena.end).toBe(SERVICES.cena.end);
  });

  it('las funciones de turno respetan horarios custom', () => {
    const custom = servicesOf({ configuration: { services: { mediodia: { start: '12:00', end: '16:00' } } } });
    expect(timeBelongsToService('11:30', 'mediodia', custom)).toBe(false);
    expect(timeBelongsToService('12:00', 'mediodia', custom)).toBe(true);
    expect(serviceFromTime('11:30', undefined, custom)).toBeUndefined();
    expect(serviceFromTime('12:30', undefined, custom)).toBe('mediodia');
    const slots = genSlots('mediodia', custom);
